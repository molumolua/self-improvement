...
exp_name=${exp_name:-"2-test-len-${history_len}-zero-${windows_continous_zero_correct_limit}-sat-${windows_continous_max_distance_limit}-W-${enable_weighted_sample}-Instruction-ne-${num_environment_per_step}-model-${model_name}-lr-${lr}-bsz-${train_prompt_bsz}-n_resp-${n_resp_per_prompt}-mini-${train_prompt_mini_bsz}"}
